...
train_aks_info_path = r'A:\huan_shit\learning_scrapy\huan_akselos\data\03_train_aks_detail_info.json'

with open(train_aks_info_path, 'r') as f:
    data = json.load(f)
    for aks_trained in data:
        job_id = aks_trained["Job ID"]
        subjob_id_parent = aks_trained["subjob_id"]
        print(subjob_id_parent)
        subjob_id_lastnode = aks_trained["Description"]
        collection_id = aks_trained['collection_id']
        crawl_page = http_pattern + job_id + '/' + subjob_id_parent + '/' + subjob_id_lastnode
        ...
        for line in text_raw:
            if 'Success' in line or 'Failure' in line or 'Running' in line:
                ...
                row_list.append({
                    ...
                    'SubjobIdParents': subjob_id_parent,
                    'subjob_id': subjob_id_lastnode,
                    'State': State,
...
file_path = r'A:\huan_shit\learning_scrapy\huan_akselos\data\04_last_node.json'
